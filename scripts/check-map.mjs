 ...
 *   npm run map
 *
 * It regenerates web/city.js in memory and compares, so a museum added to
...
const city = await readFile(new URL('web/city.js', root), 'utf8')
...
if (count !== museums.length) {
  problems.push(`web/city.js holds ${count} museums, data/museums.json has ${museums.length} — run \`npm run city\``)
}
...
const streets = await readFile(new URL('web/streets-data.js', root), 'utf8')
if (!/const ST=\{n:\[/.test(streets)) problems.push('web/streets-data.js is not in the expected shape')
const names = (streets.match(/"/g) || []).length
if (names < 1000) problems.push('web/streets-data.js looks truncated — far too few street names')
